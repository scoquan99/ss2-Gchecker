...
from models.spell_model import SpellChecker, _parse_json_response
...
def check():
    ...
    threading.Thread(
        target=save_history,
        args=(text, response, lang, username),
        daemon=True
    ).start()
...
def ai_detect():
    ...
    if not text or len(text) < 30:
        return jsonify({"ai_probability": 25, "reasoning": "Text is too short to analyze."})

    keys = get_gemini_keys()
    if not keys:
        return jsonify({"ai_probability": 35, "reasoning": "No API key configured."})

    prompt = f"""You are an expert forensic linguist specializing in detecting AI-generated text.
Analyze the following text for signs of AI authorship using these criteria:

1. LEXICAL PATTERNS: Overuse of filler phrases ("it is important to note", "in conclusion", "furthermore"), 
   repetitive sentence openers, unnaturally balanced vocabulary distribution
2. STRUCTURAL PATTERNS: Overly consistent paragraph length, formulaic structure 
   (intro → body → conclusion in every response), lack of organic digression or asides
3. SEMANTIC PATTERNS: Generic, non-specific examples; hedged language ("may", "could", "it seems");
   absence of personal anecdotes or concrete lived experience
4. STYLISTIC PATTERNS: No typos or self-corrections, perfect grammar, 
   absence of colloquialisms or rhetorical questions, suspiciously neutral tone
5. COHERENCE: AI text tends to be globally coherent but locally shallow — 
   ideas sound plausible but lack depth, originality, or strong personal opinion

Rate from 0 (definitely human) to 100 (definitely AI).
Return ONLY this JSON, nothing else:
{{"ai_probability": 70, "reasoning": "Specific evidence found in the text (cite actual phrases)"}}
...
"""
    ...
    for i, api_key in enumerate(keys):
        try:
            ...
            return jsonify(_parse_json_response(res.text))
...
def paraphrase():
    ...
    mode_instructions = {
        "standard": "Rewrite using different vocabulary and sentence structures while keeping the exact meaning.",
        "creative":  "Rewrite with creative flair — vary rhythm, use vivid language, make it more engaging.",
        "formal":    "Rewrite in formal academic/professional register. Use precise terminology, no contractions.",
        "simple":    "Rewrite using simple, everyday language. Short sentences, common words."
    }
